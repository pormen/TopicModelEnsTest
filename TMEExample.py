...
if dataset == "Dataset5": listaTop = range(10,30+1)
if dataset == "Dataset6_talk_religion_misc": listaTop = range(16,80+1)
if dataset == "Dataset6_talk_politics_misc": listaTop = range(16,80+1)
if dataset == "Dataset6_talk_politics_mideast": listaTop = range(24,120+1)
if dataset == "Dataset6_talk_politics_guns": listaTop = range(20,100+1)
if dataset == "Dataset6_soc_religion_christian": listaTop = range(28,140+1)
if dataset == "Dataset6_sci_space": listaTop = range(24,120+1)
if dataset == "Dataset6_sci_med": listaTop = range(18,90+1)
if dataset == "Dataset6_sci_electronics": listaTop = range(12,60+1)
if dataset == "Dataset6_sci_crypt": listaTop = range(18,90+1)
if dataset == "Dataset6_rec_sport_hockey": listaTop = range(18,90+1)
if dataset == "Dataset6_rec_sport_baseball": listaTop = range(18,90+1)
if dataset == "Dataset6_rec_motorcycles": listaTop = range(12,60+1)
if dataset == "Dataset6_rec_autos": listaTop = range(18,90+1)
if dataset == "Dataset6_misc_forsale": listaTop = range(12,60+1)
if dataset == "Dataset6_comp_windows_x": listaTop = range(12,60+1)
if dataset == "Dataset6_comp_sys_mac_hardware": listaTop = range(12,60+1)
if dataset == "Dataset6_comp_sys_ibm_pc_hardware": listaTop = range(12,60+1)
if dataset == "Dataset6_comp_os_ms_windows_misc": listaTop = range(14,70+1)
if dataset == "Dataset6_comp_graphics": listaTop = range(16,80+1)
if dataset == "Dataset6_alt_atheism": listaTop = range(20,100+1)
...
numTopicsTotal= 0
topicPerIt = []
resultsFile = open("C:/Mallet/testCMD/"+file.split("\\")[0]+"/"+method+"/ResultsTME.txt","w");
...
for s_it in range(1,iterations+1):
    ...
    #==== SE CUENTA EL NUMERO DE PALABRAS Y DE TEMAS QUE TIENE EL CONJUNTO COMPLETO DE MODELOS ========
    numTopicsInTME = 0;
    numWordsInTME = 0
    for it in range(s_it):
        #print "letter ", str(letters[it])
        f = open("C:/Mallet/testCMD/"+file.split("\\")[0]+"/"+method+"/Normal_SubSet_Top_Words_Matrix_"+str(letters[it])+".txt","r");
        for line2 in f:
            numWordsInTME = str(line2).split("\t");
            del numWordsInTME[-1]
            numWordsInTME =  len(numWordsInTME)
            numTopicsInTME = numTopicsInTME + 1;
        f.close()
    if debug == 0: print numWordsInTME
    if debug == 0: print numTopicsInTME



    #==== SE CREA LA MATRIZ GENERAL DE TOPICOS PALABRAS DEL CORPUS COMPLETO ========
    TMETopicWordsMatrix = numpy.zeros((numTopicsInTME,numWordsInTME))
    x = 0;
    for it in range(s_it):
        print "letter ", str(letters[it])
        ...
        for line2 in f:
            ...
            for elem in numWordsInTopic:
                TMETopicWordsMatrix[x][y] = float(elem)
                y = y + 1
            x = x + 1;
        f.close()

    if debug == 0: print TMETopicWordsMatrix



    #==== SE CREA LA MATRIZ GENERAL DE TOPICOS DOCUMENTOS DEL CORPUS COMPLETO (SIN NORMALIZAR) ========
    f = open("C:/Mallet/testCMD/"+file.split("\\")[0]+"/"+method+"/Document_Topic_Gral.txt","r");
    numDocs = 0;
    for line2 in f:
        numDocs = numDocs + 1
    f.close()

    aggrMatrixDocsTop = numpy.zeros((numDocs,numTopicsInTME))

    print "numDocs,numTopicsInTME", numDocs,numTopicsInTME
    ...
    for line2 in f:
        ...
        obj = slice(0,numTopicsInTME)
        numWordsInTopic = numWordsInTopic[obj]
        print len(numWordsInTopic)
        y = 0;
        for elem in numWordsInTopic:
            aggrMatrixDocsTop[x][y] = float(elem)
            y = y + 1
    ...
    if debug==0: print aggrMatrixDocsTop


    #==== SE CALCULA LAS PROBABILIDADES A PRIORI DE LOS TOPICOS  ========
    zTopicsPrior = aggrMatrixDocsTop/numpy.array([aggrMatrixDocsTop.sum(axis=1)]).transpose();
    if debug == 0: print zTopicsPrior
    zTopicsPrior = numpy.array([zTopicsPrior.sum(axis=0)])/zTopicsPrior.shape[0]
    if debug == 0: print zTopicsPrior


    #==== SE CREA LA LISTA DE PALABRAS DE TODO EL CORPUS ========
    ...
    for elem in wordsFileE:
        ...
        allWordsString = str(elem).split(" ")[1] + " " + allWordsString

    wordsFileE.close()
    if debug == 0: print wordList




    #==== SE CREA UN NUEVEO ARCHIVO CON LOS PSEUDODOCUMENTOS Y LAS PALABRAS ========
    matrixPeudoDocWords = numpy.matrix(TMETopicWordsMatrix);
    print matrixPeudoDocWords
    rows = matrixPeudoDocWords.shape[0]
    cols = matrixPeudoDocWords.shape[1]
    wordsFileF = open("C:/Mallet/testCMD/"+file.split("\\")[0]+"/"+method+"/matrixPeudoDocumentWords.txt","w");
    ...
    for i in range(rows):
        ...
        for j in range(cols):
            ...
            if cardinality == 0: cardinality = int(matrixPeudoDocWords.item(i,j)*100);
            if cardinality == 0: cardinality = int(matrixPeudoDocWords.item(i,j)*1000);
            if cardinality == 0: cardinality = int(matrixPeudoDocWords.item(i,j)*10000);
            if cardinality == 0: cardinality = int(matrixPeudoDocWords.item(i,j)*100000);
    ...
    #=====W, words
    #=====Y, topics
    print "NUMERO DE TEMAS Y:"
    time.sleep(0.2);
    random.seed(time.time());
    if s_it == 1: numTopics = int(numTopicsInTME/2);
    else: numTopics  = random.choice(listaTop);
    print numTopics
    n = numTopics

    new_file = file.split("\\")[0]+"/"+method+"/matrixPeudoDocumentWords.txt";
    print "===Inicio LDA para pseudo documento==="
    numTopics  = n; num_top_words = 5; alpha =0.1; beta = random = 0.01
    numTopY = numTopics
    ...
    if debug == 0: print zTopicsPrior[0]  #P(Z)
    if debug == 0: print priorsDoc  #P(Z)
    if debug == 0: print yTopicsPrior[0]  #P(Y)
    ...
    if debug == 0: print matAux


    #==== MATRIX P(Z/Y)====
    matrixTopYDocZ = matrixDocTop.transpose()*matAux
    matrixTopYDocZ = matrixTopYDocZ/numpy.array([matrixTopYDocZ.sum(axis=1)]).transpose()
    if debug == 0: print matrixTopYDocZ  #P(Z/Y)
    if debug == 0: print matrixTopYDocZ.sum(axis=1)
    ...
    aggrMatrixDocsTop = aggrMatrixDocsTop/numpy.array([aggrMatrixDocsTop.sum(axis=1)]).transpose()
    matrixDocTopY = numpy.dot(aggrMatrixDocsTop,matrixDocTop);


    print matrixDocTop  #P(Y/Z)
    print aggrMatrixDocsTop  #P(Z/D)
    if debug == 0: print matrixDocTopY   # P(Y/D)
    if debug == 0: print matrixDocTopY.sum(axis=1)
    ...
    if debug == 0: print matrixWordTop   #P(Y/W)
    if debug == 0: print matrixTopWord   #P(W/Y)

    matrixDocWords = numpy.dot(matrixDocTopY,matrixTopWord); #P(W/D) = P(Y/D)*P(W/Y)
    if debug == 0: print "#===P(W/D)"
    if debug == 0: print matrixDocWords  #P(W/D)


    topicFile = "C:/Mallet/testCMD/"+file.split("\\")[0]+"/"+method+"/TME_Results_TRAIN.txt"
    ...
    InvTopicFile = "C:/Mallet/testCMD/"+file.split("\\")[0]+"/"+method+"/TME_Results_TRAIN_inv.txt"
    ...
    print trainWords, trainWords, testDocs, itNumber, file,method, topicFile, InvTopicFile
    ...
    print "train ", float(float(trainPPL)/float(numTopY))
    print "test ", float(float(testPPL)/float(numTopY))
    print "numTopY ", numTopY

    trainValues.append(str(float(float(trainPPL)/float(numTopY))))
    testValues.append(str(float(float(testPPL)/float(numTopY))))
    topicValues.append(str(numTopY))
